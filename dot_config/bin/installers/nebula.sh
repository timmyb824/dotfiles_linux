...
download_and_extract_nebula() {
  ...
  curl -LO -s https://github.com/slackhq/nebula/releases/download/v1.9.3/nebula-linux-amd64.tar.gz
...
setup_lighthouse() {
  ...
  curl -o -s /tmp/config-lighthouse.yaml https://raw.githubusercontent.com/slackhq/nebula/master/examples/config.yml
...
setup_host() {
  ...
  # Download and configure example config
  curl -o -s /tmp/config.yaml https://raw.githubusercontent.com/slackhq/nebula/master/examples/config.yml
